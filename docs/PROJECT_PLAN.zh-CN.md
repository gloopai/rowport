...
- 单行编辑依赖 primary key；无 primary key 的表已设为只读（工具栏标识并禁用新增/编辑/删除），只能浏览。
...
- [ ] P1 结构化过滤器支持多条件。
- [ ] P1 筛选条件改为类型感知输入。
- [x] P1 更新/删除前展示将执行的 SQL 预览。（编辑/删除确认对话框内联展示实际写入的 SQL）
- [x] P1 无 primary key 表明确展示只读原因。（工具栏显示“无主键 · 只读”标识，禁用新增/编辑/删除）
- [ ] P1 支持使用唯一索引作为编辑键。
